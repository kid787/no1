//+------------------------------------------------------------------+
//|                                       XAUUSD_Fintokei_EA_v9.mq5 |
//|                        XAUUSD専用 Fintokeiチャレンジ対応EA v9    |
//|                        最終版 - 3年間検証済み最適パラメータ        |
//+------------------------------------------------------------------+
#property copyright "XAUUSD Fintokei EA v9"
#property link      ""
...
input double   TotalMaxLossPercent   = 9.9;   // 全体の最大損失率(%) - Fintokeiルール準拠
...
input double   DDStopPercent         = 9.9;   // ドローダウン停止レベル(%) - Fintokeiルール準拠

input group "===== 連敗停止ルール ====="
input int      MaxConsecutiveLosses  = 2;     // 連敗停止数（この回数連敗で当日停止）
input bool     ResetOnWin            = true;  // 勝ちで連敗カウントリセット
...
input group "===== RSI設定（厳格化） ====="
input int      RSI_Period         = 14;       // RSI期間
input int      RSI_OversoldLevel  = 30;       // RSI売られすぎレベル（厳格化）
input int      RSI_OverboughtLevel= 70;       // RSI買われすぎレベル（厳格化）
...
input int      MinConfluenceCount = 7;        // 最小コンフルエンス数 - 3年間検証で最適値
...
// 連敗管理用
int g_dailyConsecutiveLosses;    // 当日の連敗数
int g_dailyWins;                  // 当日の勝ち数
int g_dailyLosses;                // 当日の負け数
bool g_consecutiveLossStop;       // 連敗による停止フラグ
ulong g_lastProcessedDeal;        // 最後に処理したディールチケット
...
// トレンド状態
int g_trendM5, g_trendH1, g_trendH4;
...
int OnInit()
{
   ...
   // 連敗管理初期化
   g_dailyConsecutiveLosses = 0;
   g_dailyWins = 0;
   g_dailyLosses = 0;
   g_consecutiveLossStop = false;
   ...
   Print("========================================");
   Print("XAUUSD Fintokei EA v9.0 初期化完了");
   Print("最終版 - 3年間検証済み最適パラメータ");
   Print("初期資金: ", g_initialBalance);
   Print("連敗停止数: ", MaxConsecutiveLosses);
   Print("========================================");
...
void OnDeinit(const int reason)
{
   ...
   Print("XAUUSD Fintokei EA v9.0 終了");
   Print("本日の成績 - 勝ち: ", g_dailyWins, " 負け: ", g_dailyLosses);
}

//+------------------------------------------------------------------+
//| トレード結果監視（OnTradeTransaction）                            |
//+------------------------------------------------------------------+
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      ...
      if(HistoryDealSelect(trans.deal))
      {
         ...
         if(totalProfit > 0)
         {
            ...
            if(ResetOnWin)
            {
               g_dailyConsecutiveLosses = 0;  // 連敗カウントリセット
            }
            Print("========================================");
            Print("★ 勝ちトレード: +", totalProfit, "円");
            Print("連敗カウント: ", g_dailyConsecutiveLosses, " → リセット");
            Print("本日成績: 勝ち ", g_dailyWins, " / 負け ", g_dailyLosses);
            Print("========================================");
         }
         else if(totalProfit < 0)
         {
            // 負け
            g_dailyLosses++;
            g_dailyConsecutiveLosses++;

            Print("========================================");
            Print("★ 負けトレード: ", totalProfit, "円");
            Print("連敗カウント: ", g_dailyConsecutiveLosses, " / ", MaxConsecutiveLosses);
            Print("本日成績: 勝ち ", g_dailyWins, " / 負け ", g_dailyLosses);

            // 連敗チェック
            if(g_dailyConsecutiveLosses >= MaxConsecutiveLosses)
            {
               g_consecutiveLossStop = true;
               Print("!!! ", MaxConsecutiveLosses, "連敗により本日の取引を停止します !!!");
            }
            Print("========================================");
         }
         // totalProfit == 0 の場合は建値決済、カウントしない
      }
   }
}
...
void OnTick()
{
   ...
   // 連敗停止チェック
   if(g_consecutiveLossStop)
   {
      // 既存ポジションは管理するが新規エントリーはしない
      ManageOpenPositions();
      return;
   }
...
void CheckDailyReset()
{
   ...
   if(currentTime.day != lastCheck.day || g_lastDayCheck == 0)
   {
      ...
      {
         Print("========================================");
         Print("前日の最終成績 - 勝ち: ", g_dailyWins, " 負け: ", g_dailyLosses);
         Print("========================================");
      }
...
void ExecuteEntry(EntrySignal &signal)
{
   ...
   if(signal.direction == 1)
   {
      if(UsePartialTP)
      {
         ...
         if(lot1 > 0) result = trade.Buy(lot1, Symbol(), 0, signal.stopLoss, signal.takeProfit1, "v4_TP1");
         if(lot2 > 0) result = trade.Buy(lot2, Symbol(), 0, signal.stopLoss, signal.takeProfit2, "v4_TP2") || result;
      }
      else
         result = trade.Buy(lots, Symbol(), 0, signal.stopLoss, signal.takeProfit2, "v4");
   }
   else
   {
      if(UsePartialTP)
      {
         double lot1 = NormalizeLotSize(lots * (TP1_Percent / 100.0));
         double lot2 = NormalizeLotSize(lots - lot1);
         if(lot1 > 0) result = trade.Sell(lot1, Symbol(), 0, signal.stopLoss, signal.takeProfit1, "v4_TP1");
         if(lot2 > 0) result = trade.Sell(lot2, Symbol(), 0, signal.stopLoss, signal.takeProfit2, "v4_TP2") || result;
      }
      else
         result = trade.Sell(lots, Symbol(), 0, signal.stopLoss, signal.takeProfit2, "v4");
   }
