//+------------------------------------------------------------------+
//| GranvilleEA_Enhanced_v2.1.mq5 |
//| Optimized version for higher profit factor |
//| Copyright 2025, Granville EA System |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, Granville EA System Enhanced v2.1"
#property link ""
...
input int MTF_Trend_Strength = 50; // MTF移動平均の傾き最小値 (Pips)
input double Min_Distance_From_200EMA_Pips = 50.0; // 200EMAからの最小距離 (Pips)

// Input Parameters - テクニカル指標
input int EMA_Short_Period = 20; // 短期EMA期間
input int ADX_Period = 14; // ADX期間
input double ADX_Min_Level = 25.0; // ADX最小レベル (20→25に引き上げ)
input int ATR_Period = 14; // ATR期間
input double ATR_Min_Multiplier = 0.7; // ATR最小倍率 (0.5→0.7に引き上げ)
input double ATR_Max_Multiplier = 1.5; // ATR最大倍率 (2.0→1.5に引き下げ)

// Input Parameters - 利確・損切り
input double Max_SL_Pips = 200.0; // 最大損切り幅 (Pips) ★新規追加
...
// Input Parameters - ピラミッディング（増し玉）★厳格化
input bool Pyramiding_Enable = true; // 増し玉機能有効化
input int Max_Positions = 2; // 最大ポジション数 (3→2に削減)
input double Pyramiding_Min_Profit_Percent = 100.0; // 増し玉に必要な最小利益 (50→100%に引き上げ)
input double Pyramiding_Min_Distance_Pips = 100.0; // 最小追加距離 (50→100pipsに拡大)

// Input Parameters - リスク管理 ★強化
input bool DailyLossLimit_Enable = true; // 日次損失制限有効化
input double DailyLossLimit_Percent = 3.0; // 日次損失制限 (4→3%に引き下げ)
input int Max_Consecutive_Losses = 3; // 最大連敗数 ★新規追加

// Input Parameters - 時間フィルター ★最適化
input bool TimeFilter_Enable = true; // 時間フィルター有効化
input int Trade_Start_Hour = 15; // 取引開始時刻 (12→15時)
input int Trade_Start_Minute = 0; // 取引開始分
input int Trade_End_Hour = 3; // 取引終了時刻 (5→3時)
input int Trade_End_Minute = 0; // 取引終了分
input bool Monday_Enable = true; // 月曜日取引 ★新規追加
input bool Friday_Enable = true; // 金曜日取引 ★新規追加

// Input Parameters - その他
input int Max_Spread_Points = 50; // 最大スプレッド (Points) ★新規追加
input int Magic_Number = 123456; // マジックナンバー
input string EA_Comment = "Granville EA v2.1"; // EAコメント
...
int consecutiveLosses = 0; // 連敗カウンター ★新規追加
datetime lastTradeTime = 0; // 最終取引時刻
...
int OnInit()
{
    ...
    Print("========================================");
    Print("Granville EA Enhanced v2.1 初期化成功");
    Print("取引シンボル: ", Symbol_to_Trade);
    Print("リスク設定: ", Risk_Percent, "% (1ポジションあたり)");
    Print("最大ポジション数: ", Max_Positions);
    Print("最大損切り幅: ", Max_SL_Pips, " pips");
    Print("ADX最小レベル: ", ADX_Min_Level);
    Print("増し玉最小利益: ", Pyramiding_Min_Profit_Percent, "%");
    Print("日次損失制限: ", DailyLossLimit_Percent, "%");
    Print("最大連敗数: ", Max_Consecutive_Losses);
    Print("========================================");
...
void OnDeinit(const int reason)
{
    ...
    Print("Granville EA Enhanced v2.1 終了");
}
...
void OnTick()
{
    ...
    // Check consecutive losses limit ★新規追加
    if(consecutiveLosses >= Max_Consecutive_Losses)
    {
        Print("🚫 連敗制限到達: ", consecutiveLosses, "連敗 - 本日の取引停止");
        return;
    }
    ...
    // Check time filter ★曜日フィルター追加
    if(TimeFilter_Enable && !IsWithinTradingHours())
    {
        return;
    }

    // Check spread filter ★新規追加
    if(!CheckSpreadFilter())
    {
        return;
    }

    // Check volatility filter ★調整済み
...
//+------------------------------------------------------------------+
//| Check if we can add a pyramiding position ★ADX上昇確認追加 |
//+------------------------------------------------------------------+
bool CanAddPosition(ENUM_POSITION_TYPE posType)
{
    ...
    if(totalRisk > 0)
    {
        ...
        {
            Print("⚠️ 増し玉スキップ: 利益不足 (", DoubleToString(profitToRiskPercent, 1),
                  "% < ", Pyramiding_Min_Profit_Percent, "%)");
            return false;
        }
    }
    ...
    {
        Print("⚠️ 増し玉スキップ: 距離不足 (", DoubleToString(distancePips, 1),
              " pips < ", Pyramiding_Min_Distance_Pips, " pips)");
        return false;
    }

    // ★新規追加: ADXが上昇中のみ増し玉を許可
    double adx[];
    ...
    if(adx[0] <= adx[1])
    {
        Print("⚠️ 増し玉スキップ: ADX下降中 (", DoubleToString(adx[0], 1),
...
//+------------------------------------------------------------------+
//| Trend Analysis ★MTFトレンド強度フィルター追加 |
//+------------------------------------------------------------------+
int AnalyzeTrend()
{
    ...
    double mtfDiffPips = mtfDiff / pipSize;

    bool mtfUptrend = mtfDiffPips > MTF_Trend_Strength;
    bool mtfDowntrend = mtfDiffPips < -MTF_Trend_Strength;
...
//+------------------------------------------------------------------+
//| Buy Signal Check ★モメンタム・200EMA距離フィルター追加 |
//+------------------------------------------------------------------+
bool CheckBuySignal()
{
    ...
    // ★新規追加: 200EMAからの最小距離確認
    double distanceFrom200EMA = (close[0] - ma200[0]) / GetPipSize();
    if(distanceFrom200EMA < Min_Distance_From_200EMA_Pips)
    {
        Print("⚠️ 買いシグナル却下: 200EMAに近すぎる (",
              DoubleToString(distanceFrom200EMA, 1), " pips)");
        return false;
    }

    // ★新規追加: モメンタム確認
    if(!CheckRecentMomentum(true))
    {
        return false;
    }
...
//+------------------------------------------------------------------+
//| Sell Signal Check ★モメンタム・200EMA距離フィルター追加 |
//+------------------------------------------------------------------+
bool CheckSellSignal()
{
    ...
    // ★新規追加: 200EMAからの最小距離確認
    double distanceFrom200EMA = (ma200[0] - close[0]) / GetPipSize();
    if(distanceFrom200EMA < Min_Distance_From_200EMA_Pips)
    {
        Print("⚠️ 売りシグナル却下: 200EMAに近すぎる (",
              DoubleToString(distanceFrom200EMA, 1), " pips)");
        return false;
    }

    // ★新規追加: モメンタム確認
    if(!CheckRecentMomentum(false))
    {
        return false;
    }
...
//+------------------------------------------------------------------+
//| ★新規追加: Check Recent Momentum |
//+------------------------------------------------------------------+
bool CheckRecentMomentum(bool isBuy)
{
    double close[];
    ArraySetAsSeries(close, true);
    ...
    {
        // 買いの場合：直近3本が上昇トレンド
        bool momentum = (close[0] > close[2]) && (close[1] > close[3]);
        if(!momentum)
        {
            Print("⚠️ 買いモメンタム不足");
        }
...
void ExecuteBuyOrder(int positionIndex)
{
    ...
    if(trade.Buy(lotSize, Symbol_to_Trade, ask, sl, tp, comment))
    {
        ...
        Print("ロット: ", lotSize);
        Print("価格: ", ask);
        Print("損切り: ", sl, " (", DoubleToString(slPips, 1), " pips)");
...
double FindSwingLow()
{
    ...
    if(CopyRates(Symbol_to_Trade, PERIOD_H1, 0, 10, rates) < 10)
        return 0;
...
double CalculateLotSizeEnhanced(double entryPrice, double stopLoss)
{
    ...
    Print("ロット計算: 残高=", accountBalance, ", リスク=", riskAmount,
...
bool IsWithinTradingHours()
{
    ...
    // ★曜日チェック追加
    if(!Monday_Enable && currentTime.day_of_week == 1)
    {
        return false;
    }
    if(!Friday_Enable && currentTime.day_of_week == 5)
    {
        return false;
    }
...
void CheckDailyLossLimit()
{
    ...
    if(currentDate != lastResetDate)
    {
        ...
        dailyTradingAllowed = true;
        consecutiveLosses = 0; // ★連敗カウンターリセット
...
//+------------------------------------------------------------------+
//| ★新規追加: Update Consecutive Losses Counter |
//+------------------------------------------------------------------+
void UpdateConsecutiveLosses()
{
    // Check if there are closed positions since last check
    if(HistorySelect(lastTradeTime, TimeCurrent()))
    {
        ...
        for(int i = total - 1; i >= 0; i--)
        {
            ...
            if(HistoryDealGetInteger(ticket, DEAL_MAGIC) == Magic_Number &&
               HistoryDealGetString(ticket, DEAL_SYMBOL) == Symbol_to_Trade)
            {
...
bool CheckVolatilityFilter()
{
    ...
    {
        Print("⚠️ ボラティリティ低すぎ: ", DoubleToString(atrMultiplier, 2));
        return false;
    }

    if(atrMultiplier > ATR_Max_Multiplier)
    {
        Print("⚠️ ボラティリティ高すぎ: ", DoubleToString(atrMultiplier, 2));
        return false;
    }
...
{
    long spread = SymbolInfoInteger(Symbol_to_Trade, SYMBOL_SPREAD);

    if(spread > Max_Spread_Points)
    {
        Print("⚠️ スプレッドフィルター: ", spread, " > ", Max_Spread_Points, " points");
        return false;
    }
    return true;
}
