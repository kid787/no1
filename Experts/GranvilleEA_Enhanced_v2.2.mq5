//+------------------------------------------------------------------+
//| GranvilleEA_Enhanced_v2.2.mq5 |
//| Balanced filter adjustment for better profit factor |
//| Copyright 2025, Granville EA System |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, Granville EA System Enhanced v2.2"
#property link ""
...
input int MTF_Trend_Strength = 30; // MTF移動平均の傾き最小値 (Pips) 🔄v2.2: 50→30に緩和
input double Min_Distance_From_200EMA_Pips = 30.0; // 200EMAからの最小距離 (Pips) 🔄v2.2: 50→30に緩和

// Input Parameters - テクニカル指標
input int EMA_Short_Period = 20; // 短期EMA期間
input int ADX_Period = 14; // ADX期間
input double ADX_Min_Level = 22.0; // ADX最小レベル 🔄v2.2: 25→22に緩和
input int ATR_Period = 14; // ATR期間
input double ATR_Min_Multiplier = 0.6; // ATR最小倍率 🔄v2.2: 0.7→0.6に緩和
input double ATR_Max_Multiplier = 1.7; // ATR最大倍率 🔄v2.2: 1.5→1.7に緩和
...
// Input Parameters - 時間フィルター ★v2.2: 12:00-04:00に戻す
input bool TimeFilter_Enable = true; // 時間フィルター有効化
input int Trade_Start_Hour = 12; // 取引開始時刻 🔄v2.2: 15→12時に戻す
input int Trade_Start_Minute = 0; // 取引開始分
input int Trade_End_Hour = 4; // 取引終了時刻 🔄v2.2: 3→4時に変更
input int Trade_End_Minute = 0; // 取引終了分
input bool Monday_Enable = true; // 月曜日取引
input bool Friday_Enable = true; // 金曜日取引

// Input Parameters - その他
input int Max_Spread_Points = 100; // 最大スプレッド (Points) 🔄v2.2: 50→100に緩和
input int Magic_Number = 123456; // マジックナンバー
input string EA_Comment = "Granville EA v2.2"; // EAコメント
...
int OnInit()
{
    ...
    Print("========================================");
    Print("Granville EA Enhanced v2.2 初期化成功");
    ...
    Print("MTFトレンド強度: ", MTF_Trend_Strength, " pips");
    Print("増し玉最小利益: ", Pyramiding_Min_Profit_Percent, "%");
    Print("日次損失制限: ", DailyLossLimit_Percent, "%");
    Print("時間帯: ", Trade_Start_Hour, ":00-", Trade_End_Hour, ":00");
    Print("========================================");
...
void OnDeinit(const int reason)
{
    ...
    Print("Granville EA Enhanced v2.2 終了");
}
...
//+------------------------------------------------------------------+
//| ATR Volatility Filter ★v2.2: 範囲を緩和 |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Spread Filter ★v2.2: 100 pointsに緩和 |
//+------------------------------------------------------------------+
