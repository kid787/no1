...
//+------------------------------------------------------------------+
//| 【内部固定パラメータ】                                             |
//| ※最適化済みの核心ロジック - ユーザー変更不可                       |
//+------------------------------------------------------------------+
#define CONST_MIN_RISK_REWARD    1.5      // 最小リスクリワード比
#define CONST_SLIPPAGE           30       // 許容スリッページ
...
//+------------------------------------------------------------------+
//| 【資金管理設定】                                                   |
//| Fintokeiチャレンジの厳格なリスク管理ルールに準拠                    |
//+------------------------------------------------------------------+
input group "===== 資金管理設定 ====="
input double   InpInitialBalance = 0;           // 初期資金 (0=自動取得)
input double   InpRiskPercent = 0.9;            // リスク率(%) ※0.9%推奨
input double   InpMaxDailyLoss = 5.0;           // 1日最大損失(%)
input double   InpMaxWeeklyLoss = 5.0;          // 週間最大損失(%)
input double   InpMaxTotalLoss = 10.0;          // 全体最大損失(%)
input double   InpMaxPositionRisk = 3.0;        // 同時ポジションリスク上限(%)
input int      InpMaxConsecutiveLosses = 3;     // 連続損失でトレード停止 (0=無制限)
...
input string   InpSymbol = "";                  // 取引シンボル (空=自動)

//+------------------------------------------------------------------+
//| 【エントリー設定】                                                  |
//+------------------------------------------------------------------+
input group "===== エントリー設定 ====="
input bool     InpEnableLongTrades = true;      // ロング有効
input bool     InpEnableShortTrades = false;    // ショート有効
input bool     InpEnableH4Pullback = true;      // H4押し目エントリー
input bool     InpEnableH1Pullback = true;      // H1押し目エントリー
input bool     InpEnableD1Pullback = true;      // D1押し目エントリー
input bool     InpEnableH4Reversal = true;      // H4転換エントリー

//+------------------------------------------------------------------+
//| 【決済設定】                                                       |
//+------------------------------------------------------------------+
input group "===== 決済設定 ====="
input bool     InpEnablePartialTP = true;       // 分割決済有効
input ENUM_TIMEFRAMES InpTP1Timeframe = PERIOD_H1;   // TP1基準時間足
input ENUM_TIMEFRAMES InpTP2Timeframe = PERIOD_H4;   // TP2基準時間足

//+------------------------------------------------------------------+
//| 【フィルター設定】                                                  |
//+------------------------------------------------------------------+
input group "===== フィルター設定 ====="
input bool     InpUseADXFilter = true;          // トレンドフィルター有効
...
{
   REGIME_AUTO = 0,        // 自動判定
   REGIME_TREND_UP = 1,    // 強制:上昇モード
   REGIME_TREND_DOWN = 2,  // 強制:下降モード
...
{
   TACTIC_MANUAL = 0,          // 手動
   TACTIC_AUTO_DIRECTION = 1,  // 自動:方向切替
   TACTIC_AUTO_FULL = 2        // 自動:方向+リスク
};
input ENUM_TACTIC_MODE InpTacticMode = TACTIC_AUTO_DIRECTION;  // 戦術モード
input double   InpTrendUpRisk = 1.1;            // 上昇時リスク(%)
input double   InpTrendDownRisk = 1.0;          // 下降時リスク(%)
input double   InpRangeRisk = 0.0;              // レンジ時リスク(%)

//+------------------------------------------------------------------+
//| 【時間フィルター】                                                 |
//+------------------------------------------------------------------+
input group "===== 時間フィルター ====="
input bool     InpUseTimeFilter = false;        // 時間制限有効
input int      InpStartHour = 8;                // 開始時間
...
int OnInit()
{
   ...
   g_Trade.SetDeviationInPoints(CONST_SLIPPAGE);
   ...
   if(!g_TrendAnalyzer.Initialize(g_Symbol, InpUseADXFilter, CONST_ADX_PERIOD, CONST_ADX_MIN_LEVEL))
   {
   ...
   PrintFormat("[EA] Min RR: %.2f | Max Positions: %d | Max Consec Loss: %d",
               CONST_MIN_RISK_REWARD, InpMaxPositions, InpMaxConsecutiveLosses);
   PrintFormat("[EA] Long: %s | Short: %s | ADX Filter: %s",
   ...
   if(InpEnablePartialTP)
   {
      PrintFormat("[EA] Partial TP: ON | TP1=%s (%d%%) | TP2=%s",
                  EnumToString(InpTP1Timeframe), CONST_TP1_CLOSE_PERCENT,
                  EnumToString(InpTP2Timeframe));
   }
   PrintFormat("[EA] D1 Regime: %s", EnumToString(InpRegimeMode));
...
void OnTick()
{
   ...
   //--- Check minimum RR
   if(!g_LotCalculator.MeetsMinimumRR(signal.entryPrice, signal.stopLoss, signal.takeProfit, CONST_MIN_RISK_REWARD))
      return;
...
void CheckPartialTakeProfit()
{
   ...
   for(int i = ArraySize(g_PartialTPList) - 1; i >= 0; i--)
   {
      ...
      {
         // 部分決済を実行
         double closePercent = CONST_TP1_CLOSE_PERCENT / 100.0;
         ...
         if(closeLots > 0 && closeLots < currentLots)
         {
            ...
            if(closeResult)
            {
               ...
               PrintFormat("[EA] ★TP1 Partial Close★ Ticket=%I64u | Closed %.2f lots (%.0f%%) at %.5f",
                           g_PartialTPList[i].ticket, closeLots, CONST_TP1_CLOSE_PERCENT * 1.0,
...
bool CheckD1Regime()
{
   ...
   bool isTrending = g_TrendAnalyzer.IsD1TrendingMarket(CONST_D1_ADX_THRESHOLD);
